...
# Ex: 
# From the XHRs in Safari's Resource inspector, the "streams" API call uses the URL: 
#		https://api.soundcloud.com/i1/tracks/146805680/streams?client_id=02gUJC0hH2ct1EGOcYXQIzRFU91c72Ea&app_version=1471599913
...
getAnTrack()
{
	...
	trackURL=`curl -s "https://api.soundcloud.com/i1/tracks/${trackID}/streams?client_id=${client_id}&app_version=${app_version}" | perl -pe 's|.*(https://cf-media.sndcdn.com.*?)".*|$1|gi; s|\\u0026|&|gi'`
